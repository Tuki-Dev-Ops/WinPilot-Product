...
pnpm pages:shoot     # 앱마다 화면 사진 한 장씩 → apps/<앱>/docs/pages (개발 서버 필요)
pnpm overflow:check  # 네 너비에서 가로 넘침 재기 (개발 서버가 떠 있어야 함)
pnpm weight:check    # 화면이 실제로 내려받는 바이트 재기 (빌드 후 next start 기준)
pnpm bind:check      # 손님 화면이 적어 둔 값의 출처가 실재하는지 (어드민 메뉴 · store)
pnpm typecheck       # 전체 워크스페이스 타입 검사
pnpm build           # 전체 빌드
```

`weight:check`는 다른 검사와 달리 **옳고 그름이 아니라 크기**를 봅니다. 일곱 앱을 모두 빌드해 재 보면 가장 무거운 화면이 589~642KB 사이에 모입니다 — 그 대부분이 React와 Next의 실행기라 앱이 하는 일과 거의 무관하고, 우리가 얹은 몫은 50KB 남짓입니다. 코드를 세어서는 알 수 없는 값이라 실제로 띄우고 응답 본문의 길이를 더합니다 — 번들러가 안 쓰는 것을 털어 내고, 같은 조각을 여러 화면이 나눠 쓰고, `next/image`는 원본이 아니라 화면 폭에 맞춰 다시 만든 것을 내려보내기 때문입니다. 예산은 화면 갈래마다 다릅니다(`/docs/**`는 그림 도구를 함께 받습니다). **개발 서버로 재면 수가 크게 나옵니다** — 개발 빌드는 압축을 하지 않으므로 판단은 `next build` 뒤 `next start`로 합니다.

`bind:check`는 **문서가 가리키는 곳이 아직 있는지**를 봅니다. 화면마다 `admin: ['콘텐츠 > FAQ']`처럼 어느 어드민 화면이 그 값을 고치는지 적어 두는데, 그 문자열은 사람이 손으로 적는 것이라 어드민 메뉴가 바뀌어도 따라오지 않습니다. 실제로 IR 어드민에서 갈래 둘을 지웠을 때 손님 화면 열일곱이 없는 메뉴를 가리킨 채 남았고, 그때 기존 검사 넷은 모두 초록이었습니다. 고치는 화면이 없는 값은 `store: DIVIDENDS`처럼 적고, 그 이름이 `@winpilot/store`에 실제로 있는지까지 확인합니다.
