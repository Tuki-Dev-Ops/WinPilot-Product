 ...
 *
 * 이름이 화면별 묶음(`feature`·`non-functional`·`page-view`)이면 그 묶음의 **목록**을 그리고,
 * 갈래 묶음(`ia`)이면 **탭과 전체 문서**를 그린다. 한 마디 더 붙는 주소는 `[sub]` 가 받는다.
 *
...
export function generateStaticParams() {
  return [...Object.keys(DOC_TITLES), ...Object.keys(DOC_GROUPS), ...Object.keys(TAB_GROUPS)].map((doc) => ({
    doc,
  }));
}
...
export default async function DocPage({ params }: { params: Promise<{ doc: string }> }) {
  ...
  // 갈래 묶음은 묶음 자체에 본문이 있다 — `전체` 탭이 곧 `index.md` 다.
  if (TAB_GROUPS[doc]) {
    const source = readDoc('index', doc);
    if (!source) notFound();

    return (
      <DocShell active={`/${doc}`}>
        <DocTabs tabs={groupTabs(doc)} active={`/${doc}`} />
  ...
  // 화면별 묶음이면 목록을 그린다 — 묶음 자체에는 본문이 없다.
  if (DOC_GROUPS[doc]) {
          ...
          {entries.map((entry) => {
            ...
            return (
              ...
              >
                ...
                <span className="truncate font-mono text-xs text-ink-faint">{page?.route ?? `/${entry.slug}`}</span>
              </a>
            );
          })}
        </div>
