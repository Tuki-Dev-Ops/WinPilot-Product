...
/**
 * Flow Chart — **전체**. 사용자 여정과 어느 화면에서나 같은 상호작용.
 *
 * 화면 하나하나의 흐름은 `/docs/flow-chart/{화면}` 으로 나갔다. 한 장에 26개 화면의 흐름을
 * 모두 그리면 "사려면 어디를 몇 번 거치는가" 하나를 보려는 사람이 문서 전체를 훑어야 한다.
 * 여기서는 화면을 잇는 길만 보여 준다.
 *
 * 도면은 `lib/flow-specs.ts` 에서 만들어진다 — 흐름이 바뀌었을 때 고칠 곳이 하나여야
 * 도면이 화면을 따라온다.
 *
 * ## 어드민 연동
...
export default function DocsFlowChartPage() {
  return (
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-6">
        ...
        <section className="flex min-w-0 flex-col gap-3">
          ...
          <p className="text-sm leading-relaxed text-ink-muted">
            화면 하나가 아니라 <strong className="font-semibold">하고 싶은 일 하나</strong>를 따라간다. 단계는 화면
            이름으로만 적는다 — 화면 안에서 무엇을 밟는지는 그 화면의 흐름에 이미 있고, 여기까지 적으면 같은 것이
            두 벌이 된다.
          </p>
        </section>
        ...
        <section className="flex min-w-0 flex-col gap-3 border-t border-border pt-6">
          ...
          <p className="text-sm leading-relaxed text-ink-muted">
            어느 화면에서나 같은 것이다. 화면마다 되풀이해 적으면 26벌이 되고, 26벌은 한 번에 고쳐지지 않는다.
          </p>
        </section>
