 ...
 *
 * 사이드바 도면에서는 목록과 등록·상세의 관계가 보이지 않는다. 상세 화면은 메뉴에 항목이 없어
...
export default async function InternalIaScreenPage({ params }: { params: Promise<{ screen: string }> }) {
  const { screen } = await params;
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-6">
        ...
        {group && (
          <section className="flex min-w-0 flex-col gap-3">
            ...
            <p className="text-sm leading-relaxed text-ink-muted">
              이 프로젝트에는 서버가 없다. 이 콘솔이 만지는 것은 앱 안의 시드와 주소의
              질의문자열뿐이다.
            </p>
