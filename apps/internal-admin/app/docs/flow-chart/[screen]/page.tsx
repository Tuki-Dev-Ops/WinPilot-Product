...
export default async function InternalFlowChartScreenPage({ params }: { params: Promise<{ screen: string }> }) {
  const { screen } = await params;
    ...
    <div className="flex min-w-0 flex-col gap-6 lg:flex-row lg:items-start lg:gap-8">
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-6">
        ...
        <section className="flex min-w-0 flex-col gap-5">
          <List title="들어오는 길" items={flow.entries} empty="정해진 길이 없다 — 주소로 바로 들어온다." />
          ...
          />
          <List title="값이 오는 곳" items={flow.data} empty="바깥에서 읽어 오는 값이 없다." />
          <List title="나가는 길" items={flow.exits} empty="이어지는 다음 화면이 없다 — 고쳐 저장하면 그 자리에 남는다." />
        </section>
