 ...
 * 빠뜨리면 도면만 조용히 옛것이 된다. 메뉴(`internal-menu.ts`)와 갈래(`ia-groups.ts`)에서 읽어
 * 그리면 고칠 곳이 하나다.
 *
 * ## 그리는 규칙 (다른 두 앱의 문서와 같은 결)
...
export function siteMap(): string {
  const lines = [INIT, 'graph TB', '  INTERNAL["WinPilot Internal"]', ''];

  for (const group of IA_GROUPS) {
    const href = INTERNAL_MENU.find((section) => section.id === group.id)?.href;
    ...
    lines.push(`  INTERNAL --> ${node(group.id)}`);
  }

  lines.push('');
  /*
    다른 두 앱은 여기에 공유 시드(`@winpilot/store`)를 그린다. 이 콘솔은 그 패키지를 쓰지 않는다 —
    고객사를 관리하는 값은 고객 화면과 겹치지 않아서, 앱 안의 시드로만 돈다.
  */
  lines.push(`  ${cylinder('SEED', 'lib/data · 이 앱만 쓰는 시드')}`);
  lines.push('  SEED -.-> INTERNAL');
  return lines.join('\n');
}
 ...
 *
 * 사이드바 도면을 그대로 두고 화면만 칠하지 않는 이유: 최상위만 있는 도면에서는 목록과 상세의
 * 관계가 보이지 않는다. 상세 화면이 메뉴에 없다는 것을 모르면 목록에서 들어가는 길을 못 찾고
 * 같은 화면을 새로 만들려 든다.
 */
