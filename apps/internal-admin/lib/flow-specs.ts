...
export const FLOW_SPECS: ScreenFlow[] = [
  ...
  },
  {
    ...
    exceptions: [
      ...
      { at: 2, label: '다음에 할 것이 없으면 그 줄을 그리지 않는다 — 빈 줄은 정하지 않은 것과 구분되지 않는다' },
    ],
    data: ['lib/data/activities.ts', 'lib/data/tenants.ts', 'lib/data/settings.ts'],
    exits: ['타임라인 맨 위에 새 기록이 쌓인다', '고객사 상세로'],
  },
  {
  ...
  },
  {
    ...
    steps: [
      '판매 중인 플랜과 구독 중인 고객사 수를 본다',
      '플랜 카드에서 금액 · 배포 수 · 회원 상한을 읽는다',
      '켜지는 권한과 꺼진 권한을 함께 본다',
      '등록을 눌러 새 플랜을 만든다',
    ],
    branches: [
      {
        after: 3,
        question: '이름 · 월 구독료 · 회원 상한을 다 넣었나',
        block: '무엇이 비었는지 알리고 모달을 닫지 않는다',
        blockLabel: '아니오',
      },
    ],
    ...
    exits: ['새 플랜이 권한 없이 목록에 생긴다 — 무엇을 열지는 권한 화면에서 정한다'],
  },
  {
    screen: 'subscriptions-roles',
    entries: ['보조 메뉴 권한', '플랜 화면에서 권한이 무엇인지 보러'],
    steps: [
      '권한마다 여는 것을 읽는다',
      '어느 플랜부터 열리는지 본다',
      '끌 수 있는지로 거른다',
    ...
    branches: [
      {
        after: 3,
        question: '권한 이름과 여는 것을 다 적었나',
        block: '무엇이 비었는지 알리고 모달을 닫지 않는다',
        blockLabel: '아니오',
      },
    ],
    exceptions: [{ at: 0, label: '끌 수 없는 권한은 제품이 도는 데 필요한 것이라 화면에서 만들지 않는다' }],
    data: ['lib/data/subscriptions.ts'],
    exits: ['새 권한이 끌 수 있는 것으로 목록에 생긴다'],
  },
  {
    screen: 'inquiries',
    ...
    exceptions: [
      ...
      { at: 2, label: '아직 답하지 않은 문의는 빈 칸이 아니라 그렇게 적는다' },
    ],
    data: ['lib/data/inquiries.ts', 'lib/data/tenants.ts'],
    exits: ['목록을 잃지 않고 그 자리에서 읽는다'],
  },
  {
  ...
  },
  {
    ...
    steps: [
      '고객사를 고른다',
      '쓸 제공자를 켠다',
      'Client ID 와 Secret 을 넣는다',
      '자동으로 만들어진 리다이렉트 주소를 확인한다',
      '저장을 누른다',
    ],
    branches: [
      {
        after: 4,
        question: '켜 둔 제공자에 키가 다 들어 있나',
        block: '어느 제공자가 비었는지 토스트로 알리고 저장하지 않는다',
        blockLabel: '아니오',
      },
    ],
    exceptions: [
  ...
  },
  {
    ...
    entries: ['보조 메뉴 Plugin'],
    steps: ['고객사를 고른다', '얹을 조각을 켠다', '키가 필요한 조각에 값을 넣는다', '저장을 누른다'],
    branches: [
      {
        after: 3,
  ...
  },
  {
    ...
    steps: [
      '고객사를 고른다',
      '넣어야 할 레코드 목록을 읽는다',
      '값을 그대로 복사해 고객사 담당자에게 넘긴다',
      '다시 확인을 눌러 지금 상태를 읽는다',
    ],
    branches: [
      {
        after: 3,
...
export const COMMON_FLOWS: NamedFlow[] = [
  ...
  },
  {
    id: 'toolbar',
    title: '목록 툴바',
    purpose: '찾는 일과 만드는 일을 좌우로 갈라 둔다.',
    entries: ['목록 화면을 열면'],
    steps: [
      '왼쪽 검색창에 말을 넣는다',
      '그 옆 칩으로 조건을 건다',
    ...
    exceptions: [
      { at: 0, label: 'lg 미만에서는 검색이 한 줄을 다 쓰고 필터와 등록이 그 아래로 내려간다' },
      { at: 2, label: '단추 글자에 무엇을 만드는지 적는다 — 등록이 아니라 고객사 등록' },
    ],
