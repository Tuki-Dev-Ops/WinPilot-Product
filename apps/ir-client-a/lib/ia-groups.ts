...
export const IA_GROUPS: IaGroup[] = [
  {
    ...
    notes: [
      '갈래 안에서 화면끼리 잇는 길이 없다. 회사 소개 아래에 연혁·특허 및 인증으로 가던 바로가기 두 장을 뺐기 때문이다 — 그 둘은 헤더의 ABOUT 펼침에 이미 서 있고, 같은 길을 화면 안에 한 번 더 두면 화면이 끝났다는 신호가 있어야 할 자리에 링크 둘이 서서 아래가 더 있는 것처럼 읽힌다.',
  ...
  },
  {
    id: 'disclosure',
    label: '공시',
    purpose: '회사 이름으로 나간 것만 최신순으로 세우고, 한 건을 원문까지 연다.',
    screens: [
      { screen: 'disclosures', ko: '공시 정보' },
      { screen: 'disclosures-detail', ko: '공시 상세' },
    ],
    edges: [['disclosures', 'disclosures-detail']],
    data: ['@winpilot/store · DISCLOSURES (publicDisclosures · findDisclosure)'],
    notes: [
      '사이트에 서는 것은 `공시됨` 과 `정정` 뿐이다. 원고(`작성 중` · `검토 요청`)는 목록에 오지 않고 **주소로도 열리지 않는다** — 목록에서 거르는 것만으로는 번호를 짐작해 들어오는 사람을 막지 못한다(`docs/architecture/policy.md` §3).',
      '없는 공시와 아직 안 나간 공시가 화면에서 같은 404 다. 둘을 갈라 보이면 그 번호의 원고가 있다는 사실이 새어 나간다.',
      '어드민과 사이트가 같은 함수(`publicDisclosures`)로 거른다. 두 벌로 두면 어드민에서 본 것과 사이트가 갈리고, 그 어긋남은 투자자가 물어볼 때 드러난다.',
      '차례는 공시일 내림차순이고 store 가 정한다. 화면에서 다시 정렬하지 않는다.',
      '정정은 목록의 딱지와 상세의 안내 두 곳에 남는다. 무엇을 고친 것인지는 상세가 적는다 — 게시한 것은 되돌릴 수 없고 정정 공시로만 고치기 때문이다.',
      '상세에는 IR 줄(`IrSubNav`)이 없다. 목록의 자식이라 돌아가는 길 하나만 둔다.',
      '목록에 갈래 거르개도 검색도 아직 없다. 지금 건수에서는 한 화면에 다 서고, 붙일 자리는 제목 아래다.',
    ],
  },
  {
    id: 'finance',
    label: '숫자',
    purpose: '얼마를 벌었고 지금 얼마이며 주주에게 무엇이 오는지를 표로 읽는다.',
    screens: [
      { screen: 'financials', ko: '재무 정보' },
      { screen: 'stock', ko: '주가 정보' },
      { screen: 'dividends', ko: '배당 정보' },
    ],
    edges: [],
    data: [
      '@winpilot/store · FINANCIALS',
      '@winpilot/store · STOCK',
      '@winpilot/store · DIVIDENDS',
    ],
    notes: [
      '주가는 **지연 시세임을 늘 적는다.** 적지 않으면 실시간으로 읽고 그 차이로 판단한 뒤에야 안다 — 제목 줄과 숫자 옆 두 곳에 적고, 기준 시각을 숫자 바로 옆에 둔다(`docs/architecture/policy.md` §3).',
      '몇 분 지연인지는 이 화면에 적지 않는다. 시세를 어디서 받아 오는지를 정하는 것은 어드민이고, 그 값이 아직 사이트로 이어지지 않아 숫자를 적으면 지키지 못할 말이 된다.',
      '주가 숫자를 손으로 고치는 칸을 두지 않는다. 읽는 곳이라 고칠 것이 없다.',
      '금액 단위는 백만 원이다. 원 단위로 두면 자릿수가 길어 표에서 견줄 수 없고, 단위는 제목 설명에서 한 번만 말한다 — 칸마다 적으면 숫자보다 단위가 먼저 읽힌다.',
      '재무 표에 감사 전 수치가 섞일 수 있다는 것을 제목 줄에 적는다. 표만 두면 확정된 값으로 읽힌다.',
      '배당성향과 시가배당률을 화면에서 세지 않는다. store 가 든 값을 그대로 적는다 — 화면마다 세면 화면 수만큼 다른 값이 생긴다.',
      '등락은 색과 함께 화살표를 둔다. 색만으로 알리면 못 읽는 사람이 있고, 상승이 빨강인 것은 국내 관행이라 밖에서 온 사람에게는 색 자체가 반대로 읽힌다.',
      '세 화면 다 거르개도 차트도 없다. 읽는 곳이라 고를 것이 없고, 고르는 칸을 흉내만 내 두면 눌러 본 사람이 무엇이 되는지 찾는다.',
    ],
  },
  {
    id: 'shareholder',
    label: '주주',
    purpose: '언제 무엇을 의결하고 누가 회사를 맡고 있는지 본다.',
    screens: [
      { screen: 'meetings', ko: '주주총회' },
      { screen: 'meetings-voting', ko: '전자투표 안내' },
      { screen: 'governance', ko: '지배구조' },
    ],
    edges: [['meetings', 'meetings-voting']],
    data: [
      '@winpilot/store · MEETINGS',
      '@winpilot/store · OFFICERS · SHAREHOLDERS',
    ],
    notes: [
      '전자투표 화면에서 **투표하지 않는다.** 전자투표는 예탁결제원의 시스템에서 하고, 여기는 어디로 가서 무엇을 준비해야 하는지를 적는 자리다 — 투표 단추를 흉내 내 두면 눌러 놓고 표가 들어간 줄 안다(`docs/architecture/policy.md` §3).',
      '의결권 행사 기간은 총회일 전날까지다. 그 한 줄을 준비물 목록에 함께 둔다.',
      '안건을 전부 적는다. 목록에서 줄이면 주주가 무엇을 의결하는지 모른 채 참석 여부를 정하게 되는데, 이 화면에서 줄여도 되는 값이 아니다.',
      '전자투표 안내는 IR 줄(`IrSubNav`)에 없다. 아홉의 형제가 아니라 주주총회의 자식이라, 총회 카드에서만 들어가고 돌아가는 길 하나만 둔다.',
      '전자투표를 받는 총회에만 그 길을 그린다. 없는 총회에 안내 링크를 세우면 눌러 본 사람이 자기 총회가 빠진 줄 안다.',
      '사외인지 사내인지를 이름 · 직위 바로 옆에 둔다 — 지배구조에서 가장 먼저 읽는 값이다.',
      '지분 합이 100% 를 벗어나는지 재는 것은 어드민이 한다. 사이트는 받은 값을 그대로 적는다 — 두 곳에서 재면 어느 쪽이 맞는지 화면으로는 알 수 없다.',
    ],
  },
  {
    id: 'resources',
    label: '자료 · 알림',
    purpose: '더 볼 자료를 내려받고, 다음 일정을 보고, 새 공시를 메일로 받기로 한다.',
    screens: [
      { screen: 'library', ko: 'IR 자료실' },
      { screen: 'schedules', ko: 'IR 일정' },
      { screen: 'subscribe', ko: '공시 구독' },
    ],
    edges: [],
    data: [
      '@winpilot/store · IR_DOCUMENTS',
      '@winpilot/store · IR_SCHEDULES (upcomingSchedules)',
      '@winpilot/store · SUBSCRIBERS · DisclosureKind',
    ],
    notes: [
      '자료마다 파일 크기를 반드시 적는다. 모바일에서 8MB 를 모르고 누르는 것과 알고 누르는 것은 다르다(`docs/architecture/policy.md` §3).',
      '일정은 앞으로 올 것만 세운다. 지난 것을 함께 보이면 다음에 무엇이 있는지 찾기 어렵고, 자르는 일은 화면이 아니라 store 가 한다.',
      '일정은 가까운 날이 먼저다. 공시가 최신순인 것과 방향이 반대인데, 여기서 묻는 것이 다음에 무엇이 있나이기 때문이다.',
      '구독은 넣은 주소로 확인 메일을 먼저 보낸다. 남의 주소를 넣어 두고 알림을 받게 하는 것을 막는 장치이고, 확인 안 된 주소로 보내면 스팸 신고가 쌓여 보내던 메일 전체가 막힌다.',
      '구독 갈래를 고르게 한다. 전부 받으면 읽지 않게 되고, 읽지 않으면 정작 중요한 공시도 지나친다 — 고르지 않으면 전부로 본다.',
      '자료실의 파일이 아직 붙지 않았다. 제목 · 갈래 · 올린 날 · 크기는 적혀 있지만 눌러도 받아지는 것이 없다.',
      '세 화면 다 서버가 없다. 구독 신청은 토스트로 끝나고 실제로 쌓이는 곳이 아직 없다.',
    ],
  },
  {
    id: 'support',
