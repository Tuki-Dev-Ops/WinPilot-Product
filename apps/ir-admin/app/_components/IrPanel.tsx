 ...
 *
 * **목록 위 툴바는 여기 없다.** 세 어드민이 같은 것을 쓰기로 정해 `@winpilot/ui` 의
 * `ListToolbar` 로 올렸다. 여기 남은 것은 세 콘솔이 같은 모양으로 쓰되 아직 올리지 않은 카드·요약·빈 상태다 —
 ...
 */
export function IrEmpty({ children }: { children: ReactNode }) {
 ...
 * 것인지 알 수 없다.
 */
export function IrSummary({ cards }: { cards: SummaryCard[] }) {
  return (
...
export function IrTableFoot({ children }: { children: ReactNode }) {
  return (
