 ...
 * 빠뜨리면 도면만 조용히 옛것이 된다. 메뉴(`ir-menu.ts`)와 갈래(`ia-groups.ts`)에서 읽어
 * 그리면 고칠 곳이 하나다.
 *
...
export function siteMap(): string {
  const lines = [INIT, 'graph TB', '  ADMIN["IR Admin"]', ''];

  for (const group of IA_GROUPS) {
    const href = IR_MENU.find((section) => section.id === group.id)?.href;
