 ...
 *
 * 화면 하나하나의 흐름은 `/docs/flow-chart/{화면}` 으로 나갔다. 한 장에 43개 화면의 흐름을
 * 모두 그리면 "상품 하나를 올리려면 어디를 몇 번 거치는가" 하나를 보려는 사람이 문서 전체를
...
export default function AdminDocsFlowChartPage() {
  return (
    <div className="flex min-w-0 flex-col gap-6 lg:flex-row lg:items-start lg:gap-8">
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-6">
        ...
        <section className="flex min-w-0 flex-col gap-3 border-t border-border pt-6">
          ...
          <p className="text-sm leading-relaxed text-ink-muted">
            어느 화면에서나 같은 것이다. 화면마다 되풀이해 적으면 43벌이 되고, 43벌은 한 번에 고쳐지지 않는다.
          </p>
        </section>
