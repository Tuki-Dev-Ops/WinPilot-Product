/**
 * 문의 기록 — **저장된 값은 여기 한 곳에만 있다.**
 *
 * 고객 화면의 문의하기 폼은 한 곳이 아니다. 회사 소개, 상품 상세, 포트폴리오마다 같은 폼이
 * 놓이고, **어느 화면에서 보냈는지(`path`)** 를 알아야 답변의 맥락이 잡힌다. 그래서 경로를
 * 목록의 열로 두고 필터로도 쓴다.
 *
 * 어드민의 문의 목록과 고객 화면의 '문의 내역' 이 같은 기록을 본다 — 두 벌로 두면 고객이
 * 보낸 문의가 어드민에 없거나, 어드민이 단 답변이 고객 화면에 나타나지 않는다.
 */
...
/** 한 사람이 보낸 문의 — 고객 화면의 '문의 내역' 이 쓴다. 최근 것이 위로 온다. */
export function inquiriesOf(email: string): InquiryRecord[] {
  return INQUIRIES.filter((inquiry) => inquiry.email === email).sort((a, b) =>
