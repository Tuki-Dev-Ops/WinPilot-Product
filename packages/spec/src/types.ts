...
export const VIEW_META: Readonly<Record<ViewId, ViewMeta>> = {
  'b2c-client': {
    ...
    app: 'b2c-client-a',
    /*
      `3300` 이었다. 뷰의 대표 포트라는 뜻으로 둔 값인데, 그 자리에서 도는 앱이 없어
      **아무도 열 수 없는 번호**였다 — 어드민의 `사이트 보기` 링크가 그 번호를 그대로 쓰다가
      개발 중에 늘 연결 실패였다. 실제로 뜨는 자리를 적는다.
    */
    port: 3310,
    pairGroup: 'b2c',
  },
...
export const ACTIONS = [
   ...
   * 처리 결과 화면(완료 · 실패).
   *
   * `detail` 로 두지 않는 이유는 보여 주는 것이 **자원이 아니라 방금 한 일의 결말**이기
   * 때문이다. 완료와 실패를 각각 다른 동작으로 두지도 않는다 — 두 화면은 문구만 다르고
   * 구조가 같아서, 나누면 한쪽만 고쳐 두 화면이 어긋난다.
   */
  'result',
] as const;
