...
/**
 * 기획 명세(FSD)를 **만들어 낸다** — 14절짜리 문서를 화면 수만큼 손으로 적지 않는다.
 *
...
export function fsdDoc(app: FsdApp, page: PageLike, spec: ScreenSpecLike): string {
  const route = page.sampleUrl ?? page.route;

  return `# 기능 명세 — ${page.name}

> ${app.label} · 화면 id \`${spec.screen}\`
...
${table(
  ['기능', '사용자 동작', '시스템 동작'],
  spec.actions.map((action) => [action, '해당 요소를 누르거나 값을 넣는다', '조건을 확인하고 결과를 화면에 반영한다']),
  '읽기만 하는 화면입니다.',
)}
## 6. 버튼과 이벤트
...
${table(
  ['버튼', '눌렀을 때', '성공', '실패'],
  (spec.buttons ?? []).map((button) => [button.label, button.onClick, dash(button.onSuccess), dash(button.onFail)]),
...
${bullets(spec.actions.map((action) => `${action} → 결과가 화면에 바로 반영된다.`), '_화면을 열어 읽는다._')}
**예외**

${bullets(spec.guards, '_따로 막는 것이 없다._')}
## 8. 예외 처리

${table(
  ['발생 조건', '사용자 안내', '화면 처리'],
  spec.guards.map((guard) => [guard, '무엇이 왜 막혔는지 그 자리에서 알린다', '입력값과 스크롤 자리를 지킨 채 안내만 더한다']),
  '예외로 갈리는 조건이 없습니다.',
)}
## 9. 검증 규칙
...
${bullets(
  ...
  '_읽기 전용 화면이라 동작 기준 대신 표시 기준을 본다 — 아래 비기능 명세를 따른다._',
)}
## 14. 향후 확장
...
- [비기능 명세](/docs/nfs)
- [화면 캡처](/docs/page-view/${spec.screen})
